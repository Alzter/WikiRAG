...
# Negligible modifications by: Alexander Small
...
def pages_from(input):
    ...
    try: # Worst work-around to an exception ever. Don't do this! LMAO
        for line in input:
            #print(line)
        ...
        if template_file:
            if os.path.exists(template_file):
                ...
                file = fileinput.FileInput(template_file,
                                           openhook=fileinput.hook_compressed, encoding='utf-8')
                load_templates(file)
                file.close()
            else:
                if input_file == '-':
...
def main(
        ...
        templates = None,
        no_templates = False,
        revision = options.print_revision,
    ...
    if article:
        ...
        file = fileinput.FileInput(input_file, openhook=fileinput.hook_compressed, encoding='utf-8')
